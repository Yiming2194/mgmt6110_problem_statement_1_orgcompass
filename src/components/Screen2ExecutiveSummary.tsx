...
export const Screen2ExecutiveSummary: React.FC<Screen2Props> = ({
  ...
  const [selectedActionModal, setSelectedActionModal] = useState<FrictionPoint | null>(null);
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <section className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm overflow-hidden">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 mb-4">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Direct alignment between selected organizational friction and business priority outcomes. Click any action to view embedded details.
            </p>
          </div>
          <span className="text-[11px] font-semibold text-slate-400">
            3 High-Impact Initiatives
          </span>
        </div>
                  ...
                  <tr key={item.id} className={idx % 2 === 0 ? 'bg-slate-50/50' : 'bg-white'}>
                    ...
                    <td className="p-3 max-w-[220px]">
                      ...
                      <button
                        ...
                        className="mt-2 inline-flex items-center gap-1 text-[11px] font-bold text-amber-700 hover:text-amber-900 underline decoration-amber-400 hover:decoration-amber-700 transition-colors cursor-pointer"
                      >
                    ...
                    <td className="p-3 font-medium text-slate-900 max-w-[220px]">
                      ...
                      >
                        <span className="font-bold text-slate-900 group-hover:text-amber-700 group-hover:underline transition-colors block">
                        ...
                        <span className="inline-flex items-center gap-1 text-[11px] text-amber-600 font-semibold mt-1 group-hover:underline">
                          <span>Action details</span> &rarr;
                        </span>
                      </button>
                    </td>
        ...
        {/* INTERACTIVE RECOMMENDED ACTION MODAL (Embedded in section without expanding table) */}
        {selectedActionModal && (
          <div 
          ...
          >
            <div 
              className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-2xl w-full max-h-[90vh] overflow-y-auto p-5 sm:p-6 space-y-4 text-left"
            ...
            >
                ...
                <div>
                  <div className="flex flex-wrap items-center gap-2 mb-1.5">
                    <span className="px-2 py-0.5 rounded bg-amber-500 text-slate-950 font-black text-xs">
                      Rank #{rankedPoints.findIndex((p) => p.id === selectedActionModal.id) + 1} Action
                    </span>
                    <span className="text-xs font-bold px-2 py-0.5 rounded bg-slate-100 text-slate-700 border border-slate-200">
                      {selectedActionModal.priorityCode}
                    </span>
                    <span className="text-xs font-bold px-2 py-0.5 rounded bg-amber-50 text-amber-800 border border-amber-200">
                      {selectedActionModal.category}
                    </span>
                  </div>
                  <h3 className="text-lg sm:text-xl font-black text-slate-900 leading-snug">
                    {selectedActionModal.recommendedAction.title}
                  </h3>
                  <p className="text-xs text-slate-600 mt-1 font-medium">
                    <span className="font-bold text-slate-800">Priority Driven:</span> {selectedActionModal.priorityTitle}
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => setSelectedActionModal(null)}
                  className="w-8 h-8 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 flex items-center justify-center shrink-0 transition-colors cursor-pointer"
              ...
              <div className="flex flex-wrap items-center gap-2">
                <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-amber-100 text-amber-900 border border-amber-300">
                  {selectedActionModal.recommendedAction.actionCategory}
                </span>
                <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-blue-100 text-blue-900 border border-blue-200 flex items-center gap-1">
                  ...
                  {selectedActionModal.recommendedAction.implementationHorizon}
                </span>
                <span className="text-xs text-slate-500 ml-auto font-medium">
                  {selectedActionModal.headcountAffected} staff mobilized
                </span>
              </div>

              {/* Friction Context */}
              <div className="bg-slate-50 rounded-xl p-3 border border-slate-200 text-xs">
                <span className="font-bold text-slate-700 block mb-0.5">Focus Area Friction Addressed:</span>
                <p className="text-slate-600">{selectedActionModal.description}</p>
              </div>
              ...
              <div>
                ...
                <ul className="space-y-2">
                  {selectedActionModal.recommendedAction.keyDeliverables.map((deliverable, dIdx) => (
                    <li key={dIdx} className="bg-slate-50 rounded-xl p-2.5 border border-slate-200 text-xs text-slate-700 flex items-start gap-2.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
                <div className="bg-slate-50 rounded-xl p-3 border border-slate-200 text-xs">
                  <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wide block">Accountable Lead:</span>
                  <span className="font-bold text-slate-900 mt-0.5 block">{selectedActionModal.recommendedAction.leadOwner}</span>
                </div>
                <div className="bg-slate-50 rounded-xl p-3 border border-slate-200 text-xs">
                  <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wide block">Target Measurable KPI:</span>
                  <span className="font-bold text-emerald-800 mt-0.5 block">{selectedActionModal.recommendedAction.targetKpi}</span>
                </div>
              </div>
              ...
              <div className="p-3 bg-amber-50 rounded-xl border border-amber-200 text-xs text-amber-950 space-y-1">
                <span className="font-bold flex items-center gap-1.5 text-amber-900">
                  <TrendingUp className="w-3.5 h-3.5 text-amber-700" /> Strategic Alignment to Business Priority:
                </span>
                <p className="leading-relaxed text-amber-900">
                  {selectedActionModal.recommendedAction.strategicAlignment}
                </p>
              </div>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                <a
                  href={`#action-detail-rank-${rankedPoints.findIndex((p) => p.id === selectedActionModal.id) + 1}`}
                  onClick={() => setSelectedActionModal(null)}
                  className="text-xs font-bold text-amber-700 hover:text-amber-800 underline inline-flex items-center gap-1"
                >
                  <span>Scroll to full card in memo</span> &darr;
                </a>
                <button
                  type="button"
                  onClick={() => setSelectedActionModal(null)}
